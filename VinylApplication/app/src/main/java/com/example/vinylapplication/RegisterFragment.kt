...
import com.example.vinylapplication.databinding.FragmentRegisterBinding
        ...
        viewModel?.navigationStatus?.observe(requireActivity(), EventObserver { result ->
            when(result){
                NavigateStatus.SIGN_IN -> {
                    ...
                    findNavController().navigate(R.id.action_registerFragment_to_signInFragment, bundle)
                }
                NavigateStatus.REGISTER -> {
                    val bundle = bundleOf("userName" to binding.editTextUsername.text.toString())
                    findNavController().navigate(R.id.action_registerFragment_to_mainScreenFragment, bundle)
                }
                ...
                NavigateStatus.FAIL_NAME -> Toast.makeText(this.context, "Name must not be empty!", Toast.LENGTH_LONG).show()
